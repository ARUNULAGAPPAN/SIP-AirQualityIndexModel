...
def test_pm25_aqi():
    ...
    test_cases = [
        ...
        (350.4, 400, "Hazardous"),
        (500.0, 500, "Hazardous"),  # Above upper limit
    ]
    
    all_passed = True
    for conc, expected_aqi, expected_category in test_cases:
        aqi, category = pollutant_aqi(conc, "PM2.5")
        passed = aqi == expected_aqi and category == expected_category
...
def test_co_aqi():
    ...
    test_cases = [
        ...
        (40.4, 400, "Hazardous"),
        (50.0, 500, "Hazardous"),   # Above upper limit
    ]
...
def test_mq135_index():
    ...
    for conc, expected_approx, description in test_cases:
        ...
        if not passed:
            all_passed = False
        print(f"{status}: MQ135 = {conc:5.1f} ppm → Index = {index:3d} - {description}")
        if not passed:
...
def test_hardware_sensor_aqi():
    ...
    print(f"  Final AQI: {aqi1} ({contrib1.get('final_aqi', aqi1)})")
